...
if $o[:verbose]
  ...
  $i.qsa[0].qrys.each do |q|
    ...
    $tbl_qry << [q.qid, q.text ,$i.qsa.map{|e|e.stat[q.qid.to_s]['map']}, did_rl , CS_TYPES.map{|e|did_rl.scan(to_ext($top_cols[q.text][e])).size}, CS_TYPES.map{|e|$top_cols[q.text][e]}] #, gen_probs, word_cnt, no_rel_docs, no_res_docs, len_rel_docs.r2, stdev_len_rel_docs.r2,
