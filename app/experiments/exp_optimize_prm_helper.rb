...
def do_retrieval_at(xvals , yvals , o={})
  ...
  when :bm25_bf
    $template_query = :hlm
    o.merge!(:smoothing=>IndriInterface.get_field_bparam2(xvals , yvals, $k1), :hlm_weights=>([0.1]*$fields.size), :param_query=>"-msg_path='#{$bm25f_path}'", :indri_path=>$indri_path_dih)
  when :bm25_weight
    $template_query = :hlm
  ...
  qs = $i.create_query_set( get_opt_qry_name(xvals , yvals, o), o.merge(:template=>$template_query, :skip_result_set=>true, :lambda=>0.9))
