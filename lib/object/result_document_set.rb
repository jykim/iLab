...
class ResultDocumentSet < DocumentSet
  ...
  def self.create_by_merge(set_name , old_sets , o = {} , &filter)
    ...
    old_sets[0].qrys.each_with_index do |q,i|
      #debugger
      #begin
      col_score[q.qid] = $engine.get_cs_score(q, o[:cs_type], o)
    ...
    old_sets.each do |qs|
      ...
      qs.rs.docs.find_all{|e| (block_given?)? filter.call(e,qs.rs) : true }.each do |d|
        ...
        #$avg_doc_score[d.qid][qs[:template]][qs[:col_type]] = (Math.exp(qs.rs.max_score(d.qid)) + Math.exp(qs.rs.min_score(d.qid)))/2 if !$avg_doc_score[d.qid][qs[:template]][qs[:col_type]]
